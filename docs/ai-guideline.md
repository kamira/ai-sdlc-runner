...
- **Current baseline (CHG-20260703-01):** the offline store now also vendors `skills/v1.12.1` (offline `git archive` of the published skill's main HEAD `605425e`, labelled ai-sdlc v1.12.1 in its SKILL.md frontmatter — v1.12.1 has no git tag). `config/runner.yaml`'s `contract_version` default is bumped to `"1.12.1"` so new projects lock at the current published skill; existing 1.0/1.1 per-project locks are untouched and still resolve against `skills/v1.0.0`/`skills/v1.1.0`.
