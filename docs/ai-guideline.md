...
| FR-12 | Six actions are never automated at any risk grade, and no confirmation or mode relaxes them. Three layers, each able only to add a stop: **targets** (commands, paths, URLs — facts, so they outrank the declaration), the **declaration** (undeclared is refused), and the **description** against word lists (the backstop, weak by design: 6 of 18 caught, 0 of 46 ordinary briefs falsely stopped). The dry-run opt-out never covers a node that applies effects | P0 | `policy.derive`, `classify`, `PERMANENT_HALT_KINDS`, `engine._permanent_halt`, `_has_effects` |
