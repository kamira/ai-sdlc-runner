...
| FR-25 | A command whose name does not constrain what it does — an interpreter, a shell, `rm`, `curl` — **cannot be vouched for**, and settings refuse it with an explanation. The `python -m <tool>` form recognises the tool, never the interpreter | P0 | `policy.EXECUTORS`, `_MODULE_FORM`, `settings.load` |
| FR-26 | A vouch covers the **tool**, not the command line: a destructive verb in subcommand position, or a dangerous flag form, removes `ordinary` however the tool was vouched | P0 | `policy._suspect` |
...
| FR-12 | Six actions are never automated at any risk grade, and no confirmation, mode or change class relaxes them (CHG-20260827-20). Three layers, each able only to add a stop: **targets** (commands, paths, URLs — facts, so they outrank the declaration), the **declaration** (undeclared is refused), and the **description** against word lists (the backstop, weak by design: 6 of 18 caught, 0 of 46 ordinary briefs falsely stopped). The dry-run opt-out never covers a node that applies effects | P0 | `policy.derive`, `classify`, `PERMANENT_HALT_KINDS`, `engine._permanent_halt`, `_has_effects` |
...
- [ ] A declared red line stops the run with every gate confirmed and high-risk mode on, however it
      is worded; an operation declaring nothing, or an unknown kind, is refused; and the six
...
- [ ] An interpreter cannot be vouched for, in settings or at recognition time; `python -m <tool>`
      still works; and a destructive verb in subcommand position beats a vouched tool.
