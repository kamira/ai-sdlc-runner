...
| `graph` | **The flow.** 29 nodes, one kind of work each, with the module loop, the bounded retry and the feedback edge back to PM. `validate()` asserts it against `policy` | `policy` |
