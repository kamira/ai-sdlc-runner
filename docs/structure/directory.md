...
Answers: FR-12 (CLI layout), NFR maintainability.
...
│   ├── graph.py               # THE FLOW: 22 nodes, one kind of work each, designed from the flowchart
...
- **No skill content, and no module that reads one.** A test asserts it: `test_no_module_reads_a_skill`.
