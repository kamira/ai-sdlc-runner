...
| `settings.load` / `edit` / `save` | What the user sets, and where it is kept | Two fields, both about the seat floor. An unknown key is refused; a corrupt file is an error, because a typo must not read as "bypass enabled" |
...
| What text the red lines are read from | the operation's `description` **vs** the node's whole brief | The whole brief — `instructions`, `objective`, `scope`. Reading only the description meant a brief saying "deploy to production, then wipe the users table" ran to completion because the operation beside it said `ordinary`. A description giving itself away did not matter, as long as it gave itself away in the other field |
...
| Claiming the backstop works | describe it **vs** measure it | Measure it, in a test named for what it records. "Deliberately generous" was the docstring; 8 of 18 is the fact. I guessed twelve. A safety claim written as an adjective is a claim nobody has to check |
