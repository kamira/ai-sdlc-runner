...
6. **Confirm gate (before touching code)**: present the user a short summary — motivation, impact scope, the decisions **made on their behalf** (anything not derivable from the Guideline/docs/their instructions), the risk grade, and **at medium/high risk, the design diagrams** — and get their confirmation before implementing. The diagrams are required material here, not decoration: the first four are all prose and tables, while what the user actually needs to catch — "you wired that module to the wrong side", "the data flows the other way" — is exactly what prose expresses worst and what is easiest to nod through unread. **A confirmation material nobody can read buys acquiescence, not confirmation.** Draw two (an architecture diagram of the affected area + a flow diagram of this change; Mermaid by default, ASCII is fine), **show them to the user, correct them until they confirm, and only then proceed**. The user may decide not to look: write `Diagrams: skipped — <reason>` in the header; **a blank reason counts as undeclared** (a blank signature is not a signature), and **the default when nothing is declared is to draw**. Low risk / CHG-lite is exempt — bookkeeping cost has to scale with risk. This **adds no halt point**: whether the flow stops is still decided by the risk × gate matrix (low auto, medium confirm, high halt); this step only governs *what the user is shown* when it does stop. The user **reviews the risk grade here** (see grading: high-risk-list hits don't accept self-downgrades). **Pre-authorization**: the user may waive per-change confirmation for a class of changes ("this kind, just do it") — record it as a knowledge directive, written **narrow** (situation + boundary; a fuzzy boundary falls back to asking). Usage is counted in governance health; a class that misfires is **auto-revoked** (see CHG-lite). In solo sessions the AI should **proactively suggest** pre-authorization when it notices repeated confirmations of the same class. Autonomous runs use the halt contract (autonomy) instead of this interactive gate — same intent, two channels.
7. **Close acceptance in the same round (no handoff)**: once implemented, **immediately produce the matching `docs/acceptance/ACC-*.md` via `acceptance-verification` within the same round**, and update this CHG's status to "Accepted" with a link to the ACC. **Do not just mark it "pending acceptance" and stop** — in cross-session work the next session brings a new requirement, nobody comes back to do a deferred acceptance, so it hangs forever. **If items still fail, return to step 1, forming a "fix → re-implement → re-verify" loop until everything passes or the user explicitly accepts.** At close-out, also check whether this change's **motive/purpose repeats a prior CHG's** — a second occurrence triggers an autonomous **shallow knowledge record** (evidence + counters in the entry; see knowledge "autonomous triggering"). **Write the result into the CHG's `Recurrence check:` header field either way** — "no repeat" is an answer; a blank field is a skipped check. The field is what makes this step actually happen: a prose-only reminder was measurably skipped every time, so the lint requires the field on records written under v1.17+ (prospective, like every new rule).
