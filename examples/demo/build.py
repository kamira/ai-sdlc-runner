...
def index_page(demos) -> str:
    cards = "\n".join(
      ...
      <a href="{d["slug"]}.html">
        <h2>{d["title"]}</h2>
        <p>{d["blurb"]}</p>
        <span class="from">examples/{d["example"]}</span>
      </a>
    </li>''' for d in demos)
...
<div class="wrap">
  <h1>Demos</h1>
  <p class="lede">Each page is what one example's agent actually writes, inlined into a single
  file. They are generated by <code>examples/demo/build.py</code> and checked by the suite, so a
  page here cannot drift from the agent that produced it.</p>
  <ul>
__CARDS__
  </ul>
  <footer>
